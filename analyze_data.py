...
def append_row(dataframe, d):
    return dataframe.append(d, ignore_index=True)
...
def get_all_data_as_dict(f, in_folder):
    filename_data = parse_filename(f)
    f = os.path.join(in_folder, f)
    d = get_data_from_file(f)
...
def main(in_folder, out_folder):
    ...
    dataframes = {}
    for f in files:
        d = get_all_data_as_dict(f, in_folder)
